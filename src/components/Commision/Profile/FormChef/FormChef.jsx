...
const FormChef=({checked,setChecked,id})=>{
  ...
  const getFile=()=>{
    ...
    axios.get(url+'fichier/'+id+'/',config)
      ...
      const data = res.data
      setFile(data)
      if(data.Reponse_commesion==true){
        setChecked(true)
      }
      console.log(data.Reponse_commesion)
    })
    .catch((e) => {
      console.log(url)
    });
      ...
      <FormGroup>
        <Grid container spacing={3}>
         ...
         <Grid item xs={12}>
          ...
          <Grid item xs={12}>
            <FormControlLabel control={<Checkbox checked={checked} onChange={(e)=>setChecked(e.target.checked)} />} label="ShortList" />
          </Grid>
        </Grid>
      </FormGroup>
    </React.Fragment>
