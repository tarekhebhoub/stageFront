...
const Home=()=>{
  ...
  const getFiles=()=>{
    ...
    axios.get(url+'fileforDRH_Cree_Com/',config)
      .then((res) => {
      const data = res.data
      console.log(data)
      setFiles(data)
  ...
  const Files=data.map((fils,index)=>{
    return(
      <Grid key={index} item xs={4}>
        <BasicCard favorable={fils.favorable} Employee={fils.id_Emp} EmployeeNom={fils.nom} id={fils.id} Post={fils.PourPoste} Raison={fils.Raison_recrut}/>
      </Grid>
      )
    })
