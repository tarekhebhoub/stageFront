...
const Home=()=>{
  ...
  const getFiles=()=>{
    ...
    axios.get(url+'fileforDir/',config)
      .then((res) => {
      const data = res.data
      setFiles(data)
      setData(data)
  ...
  const Files=data.map((fils,index)=>{
    return(
      <Grid key={index} item xs={4}>
        <BasicCard Employee={fils.id_Emp} EmployeeNom={fils.nom} id={fils.id} Post={fils.PourPoste} Raison={fils.Raison_recrut} acceptable={fils.response_Dir}/>
      </Grid>
      )
    })
